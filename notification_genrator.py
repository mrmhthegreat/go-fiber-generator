...
def build_context(config_path: str) -> dict:
    ...
    notifications.setdefault('model',          {})
    ...
    fcm.setdefault('admin_topic_env',             'admin_default_stogne')
    fcm.setdefault('save_to_database',            False)
...
def validate_config( config: Dict[str, Any]):
    ...
    if notifications.get("websocket", {}).get("enabled", False):
        if not websocket_enabled:
            print("⚠️  WARNING: notifications.websocket.enabled is true but websocket.enabled is false")
    
    if notifications.get("triggers",[]):
            chat_enabled = config.get("chat", {}).get("enabled", False)
            for trigger in notifications.get("triggers", []):
                if trigger.get("event") in ["message_sent", "message_received"]:
                    if not chat_enabled:
...
def get_templates(config: dict, t: str, o: str) -> list[tuple[str, str]]:
    ...
    if fcm.get('enabled'):
        ...
        if fcm["controller"].get('enabled',False) or imap["web_handler"].get('enabled',False) :
    ...
    if notifications.get('controller', {}).get('enabled') :
        ...
        result.append(( f'{t}/internal/websocket/notification_logic.go.j2',   f'{o}/internal/websocket/notification_logic.go',))
    return result
