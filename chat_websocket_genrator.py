...
def build_context(config_path: str) -> dict:
    ...
    websocket.setdefault('connection',             'FCM_PROJECT_ID')
...
def validate_config( config: Dict[str, Any]):
    ...
    if chat.get("websocket", {}).get("enabled", False):
    ...
    if websocket.get("presence", {}).get("enabled", False):
        if not websocket_enabled:
    ...
    if chat.get("enabled", False):
        if not config.get("chat", {}).get("models"):
    ...
    if handler_type == "dedicated" and websocket_enabled:
        ...
        if not dedicated:
            ...
            # Validate each dedicated handler path is unique
            paths = []
            for handler in ["chat", "support", "user", "events"]:
                ...
                if cfg.get("enabled", True):
                    ...
                    paths.append(path)
        # Warn if support mode disabled but dedicated.support enabled
    ...
    for event in websocket.get("events", []):
        if event.get("enabled", True):
            handler_type_ev = event.get("handler_type", "generic")
            if handler_type_ev == "custom":
                if not event.get("custom_handler"):
    ...
    for event in websocket.get("events", []):
        if event.get("enabled", True):
            ...
            if rate_limit.get("enabled", False):
                if not rate_limit.get("max_per_minute"):
...
def get_templates(config: dict, t: str, o: str) -> list[tuple[str, str]]:
    ...
    if chat.get('enabled'):
        ...
        if chat.get('web_handler', {}).get('enabled') :
            result.append((f'{t}/internal/web/dashboard/web_chat_handler.go.j2',f'{o}/internal/web/dashboard/web_chat_handler.go',))
    else:
