...
async function submitOrderStatus(tableData) {
  ...
  try {
    const response = await htmxAjaxPromise(
      ...
      {
        target: `#ReceiptStatus-${orderId}`,
        swap: "innerHTML",
        values: {
          _csrf: csrfToken,
          receipt_status: receiptStatus,
          note: note || "",
          hasTwo: tableData.isOrderDetailsModalOpen ? "true" : "false",
          swap: "true",
        },
      }
    );
    ...
    if (response.status === 200) {
      tableData.isOrderStautdUpdateModalOpen = false;
...
async function submitDeliveryOrderStatus(tableData) {
  const form = document.getElementById("update-order-delivey-status-form");
  if (!form) {
    showNotification("Form not found", "error");
    return;
  }
  ...
  if (!receiptStatus) {
    showNotification("Delivery status is required", "error");
    return;
  }
  ...
  try {
    ...
    // Handle success
    if (response.status === 200) {
      tableData.isOrderDeliveryStautdUpdateModalOpen = false;
      showNotification("Order updated successfully", "success");
    } else {
      const message = response.responseText
        ? JSON.parse(response.responseText).error || "Failed to update status"
...
function orderTableData() {
  return {
    ...
    isLoading: false,
    totalorderLoaded: 0,
    isOrderStautdUpdateModalOpen: false,
    isOrderDeliveryStautdUpdateModalOpen:false,
    ...
    submitOrderStatus() {
      window.submitOrderStatus(this);
    },
    submitOrderDeliveryStatus() {
      window.submitDeliveryOrderStatus(this);
    },
    openOrderDetailsView(orderId) {
      this.currentOrderID = orderId;
      this.isOrderDetailsModalOpen = true;
      this.isReciptModalOpen = false;
      this.isOrderStautdUpdateModalOpen = false;
      this.isOrderDeliveryStautdUpdateModalOpen = false;
      this.isDeleteModalOpen = false;
      this.isLoading = true; // Set loading state

      window.loadOrderDetail(orderId, this);
    },
    openOrderUpdateView(orderId) {
      this.currentOrderID = orderId;
      this.isReciptModalOpen = false;
      this.isOrderDeliveryStautdUpdateModalOpen = false;
      this.isOrderStautdUpdateModalOpen = true;
      this.isDeleteModalOpen = false;
    },
    openOrderDeleiveryUpdateView(orderId) {
      this.currentOrderID = orderId;
      this.isReciptModalOpen = false;
      this.isOrderStautdUpdateModalOpen = false;
      this.isOrderDeliveryStautdUpdateModalOpen = true;
      this.isDeleteModalOpen = false;
    },
    openReciptView(src) {
